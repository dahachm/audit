...
6) Регистрация событий запуска системы
   ...
   Например, чтобы посмотреть в какое время запускалась система, с помощью следующей команды:
   
   ```
   $ cat /var/log/messages.1 | grep '/usr/sbin/parlogd: start daemon'
   ```
   Так как демон регистрации событий подсистемы parsec **parlogd**, как правило, запускается вместе с системой, то по времени его старта можно отслеживать врем я старта системы.
...
> В данном примере мы создается шаблон с названием *RemoteLogs*, который принимает логи всех категорий, любого уровня; 
> 
> логи, полученные по данному шаблону будут сохраняться в каталоге по маске /var/log/rsyslog/<имя компьютера, откуда пришел лог>/<приложение, чей лог пришел>.log; 
> 
